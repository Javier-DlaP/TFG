...
Teniendo las bounding boxes 3D transformadas a bounding boxes 2D se marcan aquellas que son visibles desde la cámara y cuales no, para ello se define las bounding boxes 2D como (-1, -1, -1, -1) para identificar que hay un objeto visibles desde el vehículo pero no desde la posición de la cámara.
...
\item \ac{ave}\par
La evaluación de los sistemas de seguimiento también se han tenido en cuenta por lo que se calcula una métrica como es el \acs{ave} para saber de que manera se estima la velocidad de los objetos, pudiendo obtenerla tanto por clase como por la media de todas estas. Esta métrica se calcula como el error de velocidad absoluto en 2D trabajando en \acs{bev}.
\end{itemize}
